...
def test_performance_select_like(db_connection):
    _, no_index_duration = execute_query(db_connection, "SELECT addr FROM test1 WHERE addr LIKE '' ORDER BY addr ASC;")
    print('\nВремя запроса SELECT для столбца без индекса:',no_index_duration)
    _, index_duration = execute_query(db_connection, "SELECT addr_ind FROM test1 WHERE addr_ind LIKE '' ORDER BY addr_ind ASC;")

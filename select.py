...
# Функциональный тест (для примера оставлено select ... like '%1234')
def test_functional_select_like(db_connection):
    no_index_results, _ = execute_query(db_connection, "SELECT addr FROM test1 WHERE addr LIKE '%1234' ORDER BY addr ASC;")
    index_results, _ = execute_query(db_connection, "SELECT addr_ind FROM test1 WHERE addr_ind LIKE '%1234' ORDER BY addr_ind ASC;")
...
def test_performance_select_like(db_connection):
    _, no_index_duration = execute_query(db_connection, "SELECT addr FROM test1 WHERE addr LIKE '%1234' ORDER BY addr ASC;")
    print('\nВремя запроса SELECT для столбца без индекса:',no_index_duration)
    _, index_duration = execute_query(db_connection, "SELECT addr_ind FROM test1 WHERE addr_ind LIKE '%1234' ORDER BY addr_ind ASC;")
